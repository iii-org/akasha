...
請在與 `agent_step3.py` 相同的資料夾建立這個小型 Skill：
...
建立 `hello-skill/scripts/greet.py`，內容如下：
...
接著在與 `hello-skill` 相同的資料夾建立 `agent_step3.py`：
...
在包含 `agent_step3.py` 與 `hello-skill` 的資料夾執行：
...
- 找不到 Skill：確認 `hello-skill/SKILL.md` 存在，並改用絕對 Skill 路徑。
