...
Create this small Skill in the same folder as `agent_step3.py`:
...
Create `hello-skill/SKILL.md` with this content:
...
2. Pass a list containing exactly one name as its argument.
3. Return the script stdout as the final answer.
4. If it fails, report the exit code and stderr.
```

Create `hello-skill/scripts/greet.py` with this content:
...
Now create `agent_step3.py` in the same folder as `hello-skill`:
...
Run it from the folder containing `agent_step3.py` and `hello-skill`:
...
- Skill not found: check that `hello-skill/SKILL.md` exists and use an absolute Skill path.
