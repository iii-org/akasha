...
from akasha.utils.prompts.gen_prompt import default_self_ask_prompt, OutputSchema, JSON_formatter, format_sys_prompt
...
def self_ask_f(self: atman, start_time: float, timestamp: str):
    ...
    prod_sys_prompt = format_sys_prompt(self_ask_sys_prompt, self_ask_prompt,
    ...
    text_input = format_sys_prompt(
        self.system_prompt, inter_info + "User Question: " + final_prompt,
        self.prompt_format_type, self.model)
...
def get_inter_info(inter_q, inter_a) -> str:
    ...
    return "Use follow information to answer User Question\n" + "\n----------------------\n".join(
        inter_info)
