...
def _my_experts(EXPERTS, username):
    ...
    # st.write(EXPERTS)
    col_name, col_data, col_embedding, col_chunksize, col_action = st.columns([2, 2, 3, 1, 2])
    ...
    for expert_name in EXPERTS:
        col_name, col_data, col_embedding, col_chunksize, col_delete = st.columns([2, 2, 3, 1, 2])
        ...
        dataset_files, embedding_model, chunk_size, _ = get_expert_info(expert_owner, expert_name)
        
        flatten_filelist = []
        for df in dataset_files:
            expander_name = df['name'] if df['owner'] == username else f'{df["name"]}@{df["owner"]}'
        ...
        col_delete.button('Delete', f'btn-delete-{expert_owner}-{expert_name}', type='primary', 
...
def _new_expert(EMBEDDING_MODELS, DATASETS, username, all_users):
    ...
    new_expert_chunksize = col_chunksize.number_input('Chunk Size', min_value=10, value=500, step=1, help='Max number of texts to be chunked')
    st.markdown('')
    ...
    with st.expander('Select files', expanded=False):
        ...
        for dataset in new_expert_datas:
            ...
            if st.session_state.get('expert-add-files') is None:
                st.session_state['expert-add-files'] = dict()
            odataset, owner = dataset.split('@') if check_dataset_is_shared(dataset) else (dataset, username)
            for filename in filelist:
                
                col_filename.text(filename)
                last_update = get_lastupdate_of_file_in_dataset(odataset, filename, owner)
                ...
                checked = col_select.checkbox('select dataset file', key=f'checkbox-{dataset}-{filename}', value=True, label_visibility='collapsed')
                if checked:
    ...
    if create_expert_button:
        if new_expert_name == '' or new_expert_embedding=='' or new_expert_chunksize=='' or len(new_expert_datas)==0:
            st.warning('Please input expert information completely')
        else:
            res1 = create_expert(username, new_expert_name, new_expert_embedding, new_expert_chunksize, st.session_state['expert-add-files'])
            if res1:
...
def _update_expert(EXPERTS, EMBEDDING_MODELS, DATASETS, username, all_users):
    ...
    if len(EXPERTS) == 0:
        ...
        if editing_expert_name:
            dataset_files, embedding_model, chunk_size, default_expert_shared_users = get_expert_info(username, editing_expert_name)
            ...
            st.subheader('1. Expert Info', divider='grey')        
            col_name, col_embedding, col_chunksize = st.columns([2, 3, 1])
            update_expert_name = col_name.text_input('Name', placeholder='new name', help='Name of expert', value=editing_expert_name)
            ...
            update_expert_chunksize = col_chunksize.number_input('Chunk Size', min_value=10, value=default_expert_chunksize, 
                                                                help='Max number of texts to be chunked')
            # datasets
            st.subheader('2. Dataset(s)', divider='grey')
            col_share, col_datasets = st.columns([1, 5])
            not_shared_datasets = list_datasets(username, name_only=True, include_shared=False)
            ...
            default_expert_datasets = get_datasets_of_expert(username, dataset_files, candidate_datasets)
            ...
            with st.expander('Select files'):
                ...
                for dataset_name in expert_dataset_names:
                    ...
                    for filename in filelist:
                        col_filename.text(filename)
                        last_update = get_lastupdate_of_file_in_dataset(dataset_name, filename, dataset_owner)
                        ...
                        file_used_by_expert = check_file_selected_by_expert(dataset_files, dataset_name, dataset_owner, filename)
                        checked = col_select.checkbox(' ', key=f'checkbox-{dataset_owner}-{dataset_name}-{filename}', value=file_used_by_expert, label_visibility='collapsed')
                        if checked:
