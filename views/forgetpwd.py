    ...
    if url_params['forgetpwd']:
        try:
            ...
            if username_forgot_pw is None:
                ...
                if username_forgot_pw in list(config['credentials']['usernames'].keys()):
                    st.write(config)
                    with open(ACCOUNTS_PATH, 'w') as file:
                        yaml.dump(config, file, default_flow_style=False)
                    st.success(f'Your new password is "{random_password}", remember to change your password after login.')
                else:
