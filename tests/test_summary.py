...
@pytest.fixture
def base_line():
    sum = summary.Summary(verbose=False, chunk_overlap=41, chunk_size=501, max_token=3010, temperature=0.15)
...
def test_Summary(base_line):
    ...
    text = sum.summarize_file( file_name=file_path, summary_type="map_reduce", summary_len=100)
    
    assert type(text) == str
    
    
    text = sum.summarize_file( file_name=file_path, summary_type="refine", summary_len=100)
    assert type(text) == str
