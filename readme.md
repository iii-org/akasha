...
Make sure to deploy all the models in [Azure OpenAI Studio](https://oai.azure.com/portal). The deployment name should match the model name. Save the following information in a `.env` file in your current working directory:
...
openai_model = "openai:gpt-3.5-turbo"            # GPT-3.5 (API Key needed)
openai4_model = "openai:gpt-4"                    # GPT-4 (API Key needed)
openai4o_model = "openai:gpt-4o"                  # GPT-4o (API Key needed)

# Hugging Face Models (require environment variable 'HUGGINGFACEHUB_API_TOKEN')
huggingface_model = "hf:meta-llama/Llama-2-7b-chat-hf"  # Meta Llama-2 model (requires API token to download)
