...
請確保已在 [Azure OpenAI Studio](https://oai.azure.com/portal) 中部署所有模型，部署名稱應與模型名稱一致。將以下資訊儲存在 `.env` 文件中：
...
ao = akasha.test_agent(verbose=True, tools=[input_tool, akasha.get_saveJSON_tool()], model="openai:gpt-3.5-turbo")
